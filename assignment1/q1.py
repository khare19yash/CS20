...
# YOUR CODE
matrix = tf.random_uniform([10,10])
matrix_det = tf.matrix_determinant(matrix)
...
def f1(): tf.reduce_mean(tf.square(x - y))
def f2(): tf.abs(x-y)
out = tf.cond(tf.reduce_mean(x-y) < 0.0,f1,f2)
print(sess.run(out))
