...
class StyleTransfer(object):
    ...
    def _content_loss(self, P, F):
        ...
        ## TO DO
        shape = P.shape
        F = tf.transpose(F,perm=(0,3,1,2))
        F = tf.reshape(F,[shape[0]*shape[3],shape[1]*shape[2]])
        P = tf.transpose(P,perm=(0,3,1,2))
        P = tf.reshape(P,[shape[0]*shape[3],shape[1]*shape[2]])
        self.content_loss = tf.reduce_sum((F - P)**2) / (4 * shape[1] * shape[2] * shape[3])
        ###############################
    ...
    def _gram_matrix(self, F, N, M):
        ...
        F = tf.transpose(F,perm=(0,3,1,2))
        F = tf.reshape(F,[M,N])
        gram_mat = tf.matmul(F,tf.transpose(F))
        return gram_mat
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        E_l = tf.reduce_sum(tf.squared_difference(a_gram,g_gram)) / (4 * (N**2) * (M**2))
        return E_l
        ###############################
            ...
            for index in range(initial_step, n_iters):
                ...
                if (index + 1) % skip_step == 0:
                    ...
                    if (index + 1) % 20 == 0:
                        ...
                        ###############################
                        saver.save(sess,'chaeckpoints/style_transfer/style_transfer',index)
                        pass
